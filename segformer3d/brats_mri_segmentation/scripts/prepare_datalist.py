...
def produce_datalist(dataset_dir: str, train_size: int = 200):
    ...
    samples = sorted(glob.glob(os.path.join(dataset_dir,'data3D','train_data',"*"), recursive=True))
    datalist = []
    for line in samples:
...
def main(args):
    ...
    datalist = produce_datalist(dataset_dir=grand_parent_dir, train_size=args.train_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=str, default="brats_mri_segmentation/configs/datalist.json", help="relative path of output datalist json file."
    )
