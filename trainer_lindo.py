...
class Brats2018_Trainer:
    ...
    def _train_step(self):
        ...
        with tqdm(total=len(self.train_loader), desc=f"Epoch {self.current_epoch + 1}/{self.num_epochs} Rank:{torch.distributed.get_rank()}", unit='batch') as pbar:
    ...
    def _val_step(self):
        ...
        self.model.eval()
        with torch.no_grad():
            with tqdm(total=len(self.valid_loader), desc=f"Validation", unit='batch') as pbar:
                for idx, raw_data in enumerate(self.valid_loader):
                    ...
                    pbar.set_postfix({'val_loss': epoch_avg_loss, 'mean_dice': total_dice})
                    pbar.update(1)
